...
def test_clean_bloomberg():
    ...
    assert clean_bbg.roll_over(dt.date(), date_range, date_ranges, 0) == (
        datetime(2010, 3, 19).date(),
        datetime(2010, 6, 18).date(),
    )
...
def test_pull_optionm_api_data():
    index_name = "SPX"
    optionm_df = pull_optionm.pull_index_implied_dividend_yield(
        index_name,
...
def test_filter_index_implied_dividend_yield():
    ...
    assert (grouped == 2).all().all(), (
        "Not all groups have a count of 2 for 'expiration' and 'rate'."
    )
    assert filtered_df["expiration"].apply(lambda d: d.weekday() != 5).all(), (
        "Some expiration dates fall on Saturday."
    )
...
def test_latex_report():
    ...
    assert len(tex_files) > 0, (
        f"Found {len(tex_files)} .tex file(s) in _output directory."
    )
...
def test_spread_correlation(threshold=0.8):
    ...
    ndx = pd.concat(
        [expected["Eq_SF_NDAQ"], check["NDX_arb_spread"]], axis=1, join="inner"
    )
    spx = pd.concat(
        [expected["Eq_SF_SPX"], check["SPX_arb_spread"]], axis=1, join="inner"
    )
    djx = pd.concat(
        [expected["Eq_SF_Dow"], check["DJI_arb_spread"]], axis=1, join="inner"
    )
